    ...
    switch (node.type) {
        ...
        case 'BlockStatement': {
            ...
            if (node.body[node.body.length - 1] &&
                node.body[node.body.length - 1].generaltype !==
                    'Statement' &&
                node.body[node.body.length - 1].type !==
                    'AssignmentExpression') {
                lastNode = node.body.pop();
            }
            return `{${node.body.map(exports.generate).join('\n')}${lastNode ? `\nreturn ${exports.generate(lastNode)}` : ''}}`;
        }
        case 'IfStatement':
            return `if (${exports.generate(node.test)}) ${exports.generate(node.consequent)} ${node.alternate
                ? `${exports.generate(node.alternate)}`
                : ''}`;
        case 'VariableDeclaration':
            return `${node.kind} ${node.declarations.map(exports.generate)};`;
